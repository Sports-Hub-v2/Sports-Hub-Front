...
import type { User, UserProfileUpdateDto, PublicUserProfileResponseDto } from "@/types/user";
...
export const getProfileByAccountIdApi = async (accountId: number): Promise<User> => {
  const res = await axiosInstance.get<User>(`${API_USERS_BASE_URL}/profiles/by-account/${accountId}`);
  return res.data as User;
};

// Create profile (first-time setup)
export const createProfileApi = async (payload: Partial<User> & { accountId: number; name: string }): Promise<User> => {
  const res = await axiosInstance.post<User>(`${API_USERS_BASE_URL}/profiles`, payload);
  return res.data as User;
};

// Not implemented yet server-side — keep API surface for later
export const updateMyProfileApi = async (profileId: number, updatedData: Partial<UserProfileUpdateDto>): Promise<User> => {
  const res = await axiosInstance.patch<User>(`${API_USERS_BASE_URL}/profiles/${profileId}`, updatedData);
  return res.data as User;
};

export const fetchPublicUserProfileApi = async (_userId: number | string): Promise<PublicUserProfileResponseDto> => {
  throw new Error("공개 프로필 조회 API는 아직 준비 중입니다.");
};

export const getUserTeamsApi = async (accountId: number | string): Promise<TeamSummary[]> => {
  try {
    const prof = await getProfileByAccountIdApi(Number(accountId));
    const profileId = (prof as any).id as number;
    
    // 멤버십 조회
    const memRes = await axiosInstance.get(`/api/teams/memberships/by-profile/${profileId}`);
    ...
    for (const membership of memberships) {
      try {
        const teamId = membership?.id?.teamId ?? membership.teamId;
        if (!teamId) continue;
        ...
        teams.push({
          id: (team as any).id,
          name: (team as any).teamName ?? (team as any).name ?? '팀명 없음',
          logoUrl: null,
          region: (team as any).region ?? '',
          roleInTeam: membership.roleInTeam ?? '멤버',
        });
      } catch (teamError) {
        console.warn(`팀 ${membership.teamId} 정보 조회 실패:`, teamError);
...
export const getUserPostsApi = async (_userId: number | string): Promise<PostType[]> => {
  // 입력은 accountId로 받고, 내부에서 profileId로 변환 후 조회
  const prof = await getProfileByAccountIdApi(Number(_userId));
  const profileId = (prof as any).id as number;
  const res = await axiosInstance.get(`/api/recruit/posts`, { params: { writerProfileId: profileId } });
