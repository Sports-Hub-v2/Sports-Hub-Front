...
const MercenaryCard = ({ post, onClick }: Props) => {
  ...
  const getStatusBadge = () => {
    switch (post.status) {
      case 'RECRUITING':
        return <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">모집중</span>;
      case 'COMPLETED':
        return <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">모집완료</span>;
      default:
        return null;
    }
  };
    ...
    >
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex justify-between items-center">
          <div className="text-xs text-blue-600 font-medium">{getPostTypeLabel()}</div>
          {getStatusBadge()}
        </div>
        
        {/* 제목 */}
        <h3 className="text-lg font-semibold truncate text-gray-900">{post.title}</h3>
