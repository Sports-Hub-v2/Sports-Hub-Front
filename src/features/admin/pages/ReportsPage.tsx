...
const ReportsPage = () => {
  ...
  // Fetch backend reports on mount
  useEffect(() => {
    const loadBackendReports = async () => {
      ...
      try {
        ...
        setBackendError(error instanceof Error ? error.message : 'Failed to fetch reports');
      } finally {
          ...
          <div>
            <div style={{ fontWeight: '600', fontSize: '14px' }}>
              {isLoadingBackend ? '백엔드 데이터 로딩 중...' :
               ...
               useBackendData ? `실제 데이터 표시 중 (${backendReports.length}개 신고)` :
               `목업 데이터 표시 중`}
            </div>
            <div style={{ fontSize: '12px', opacity: 0.9 }}>
              {isLoadingBackend ? 'API 호출 중입니다...' :
               backendError ? `오류: ${backendError}` :
               useBackendData ? `백엔드 API에서 ${backendReports.length}개의 신고 데이터를 가져왔습니다` :
