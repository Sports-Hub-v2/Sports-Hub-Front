...
const MercenaryPostModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const [recruitmentFlow, setRecruitmentFlow] =
    useState<RecruitmentFlow>("TEAM_TO_INDIVIDUAL");
  ...
  useEffect(() => {
    if (initialData && isOpen) {
      ...
      setRecruitmentFlow(
        initialData.fromParticipant === "INDIVIDUAL"
          ? "INDIVIDUAL_TO_TEAM"
          : "TEAM_TO_INDIVIDUAL"
      );
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      console.error("[DEBUG] 필수 필드 검사 실패!");
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setRecruitmentFlow(e.target.value as RecruitmentFlow)
            }
            className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
          >
