...
// 포지션 한글 매핑 (Downloads 버전에서 가져온 디자인 요소)
const POSITION_LABEL_MAP: { [key: string]: string } = {
    'FW': '공격수',
    'MF': '미드필더',
    'DF': '수비수',
    'GK': '골키퍼',
...
const MercenaryDetailCard: React.FC<MercenaryDetailCardProps> = ({
  ...
  // 포지션 데이터 파싱 (Downloads 버전에서 가져온 로직)
  // post.positions가 문자열이면 객체로 변환
    let positionsObject: { [key: string]: number } | null = null;
    if (post.preferredPositions) {
        if (typeof post.preferredPositions === 'string') {
            try {
                // 백엔드에서 받은 JSON 문자열을 객체로 파싱
                positionsObject = JSON.parse(post.preferredPositions);
            } catch (e) {
                console.error("positions 데이터를 파싱하는 데 실패했습니다:", post.preferredPositions, e);
            }
        } else if (typeof post.preferredPositions === 'object') {
            // (이미 객체인 경우 - 예: 방금 생성한 DTO)
            positionsObject = post.preferredPositions;
        }
    }

  // post.positions 데이터를 표시용 문자열로 가공
  let positionsDisplay: string | null = null;
  ...
  if(positionsObject){
      if(positionsObject?.ALL && positionsObject.ALL > 0){
          ...
          // 개별 포지션이 선택된 경우 (0명 초과인 포지션만 필터링)
          const activePositions = Object.entries(positionsObject)
          ...
          if (activePositions.length > 0) {
              positionsDisplay = activePositions.join(', '); // 쉼표로 연결
          }
      }
  }
    ...
    <div className="bg-white shadow-xl rounded-xl overflow-hidden col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4">
      ...
      <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-gray-50 p-4 rounded-lg">
            ...
            {positionsDisplay && (
              <div>
                ...
                <p className="font-semibold text-gray-900">{positionsDisplay}</p>
              </div>
            )}
            {post.ageGroup && (
              <div>
        ...
        <div className="flex flex-wrap gap-3 justify-end pt-4 border-t border-gray-200">
          {/* 신청 관련 버튼 */}
          {onApply && (post.status === "RECRUITING" || post.status === "OPEN") && (
            <>
              {isAlreadyApplied ? (
